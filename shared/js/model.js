(function() {
  ...
  Skeletor.Model = (function() {
    function Model() {}
    Model.requiredCollections = ['brainstorms', 'tags', 'states', 'notes', 'relationships', 'projects', 'habitats', 'graphs'];
    ...
    Model.defineModelClasses = function(username) {
      ...
      this.Graph = this.db.Document('graphs').extend({
        defaults: {
          'created_at': new Date(),
          'modified_at': new Date(),
        }
      ...
      this.Graphs = this.db.Collection('graphs').extend({
        model: Skeletor.Model.Graph
      });
    };
